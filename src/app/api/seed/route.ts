...
const CLIENTS = [
  ...
  { id: 6,  name: 'Palais des Thes',     currency: 'USD', model: 'TIERED_REL',   status: 'Active',   fixed_fee: 0,     per_unit_rate: 0,    min_fee: 0,    max_fee: 0,    tiers: [{limit:500,price:1000},{limit:1000,price:1800},{limit:2000,price:3200},{limit:3000,price:4200},{limit:4000,price:4800}], addon1: 0, addon2: 0, twilio_sid: null, notes: 'Rel tiers' },
...
export async function GET() {
  ...
  if (existing && existing.length > 0) {
    return NextResponse.json({ message: 'Already seeded', clients: existing.length })
  }

  // Insert clients
  const clientRows = CLIENTS.map(c => ({ ...c, tiers: JSON.stringify(c.tiers) }))
  const { error: clientError } = await supabase.from('clients').insert(clientRows)
  if (clientError) return NextResponse.json({ error: 'clients: ' + clientError.message }, { status: 500 })

  // Insert settings
  await supabase.from('settings').upsert([
    { key: 'exchange_rate', value: '3.6' },
    { key: 'twilio_cost_per_sms', value: '0.007' },
    { key: 'sms_bill_rate', value: '0.008' },
  ], { onConflict: 'key' })

  // Insert historical revenue
  const revenueRows: { client_id: number; month: string; calculated_usd: number; revenue_status: string }[] = []
  for (const [clientId, currency, ...values] of HISTORICAL) {
    MONTHS.forEach((month, i) => {
      ...
      revenueRows.push({ client_id: clientId as number, month, calculated_usd: usd, revenue_status: status })
    })
  }

  const { error: revError } = await supabase.from('monthly_revenue').insert(revenueRows)
  if (revError) return NextResponse.json({ error: 'revenue: ' + revError.message }, { status: 500 })

  return NextResponse.json({ message: 'Seeded successfully', clients: CLIENTS.length, revenueRows: revenueRows.length })
}
